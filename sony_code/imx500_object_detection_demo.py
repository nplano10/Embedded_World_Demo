...
class Detection:
    def __init__(self, coords, category, conf, metadata,imx500_object_detector):
        """Create a Detection object, recording the bounding box, category and confidence."""
        self.category = category
        self.conf = conf
        self.box = imx500_object_detector.imx500.convert_inference_coords(coords, metadata, imx500_object_detector.picam2)

class IMX500ObjectDetector:
    def __init__(self, args,camera_index):
    ...
    def parse_detections(self,metadata: dict):
        ...
        if self.intrinsics.postprocess == "nanodet":
            ...
            if  self.args.model_type == "sony":
                boxes, scores, classes = np_outputs[0][0], np_outputs[1][0], np_outputs[2][0]  # for existing compiled models
            elif self.args.model_type =="sue": 
                boxes, classes, scores = np_outputs[0][0], np_outputs[1][0], np_outputs[2][0]  # for sony's BB model
        ...
        self.last_detections = [
            Detection(box, category, score, metadata,self)
